...
#	{id => 4, fake => 0, label => 'logique radio',  sql_type => 'TINYINT', max_len => 0},
#	{id => 5, fake => 0, label => 'long texte',     sql_type => 'TEXT',    max_len => 0},
#	{id => 6, fake => 0, label => 'pièce jointe',   sql_type => 'VARCHAR', max_len => 0},
#	{id => 7, fake => 0, label => 'logique case',   sql_type => 'TINYINT', max_len => 0},
	{id => 8, fake => 0, label => 'plusieurs cases',   sql_type => 'TEXT', max_len => 0},
],
